...
CREATE TABLE IF NOT EXISTS health_summary (
  ...
  last_version_change TEXT DEFAULT NULL,
  recent_breaking_change INTEGER DEFAULT 0,
  updated_at TEXT DEFAULT (datetime('now'))
);
...
CREATE INDEX IF NOT EXISTS idx_watch_key ON tool_watches(api_key_hash);
CREATE INDEX IF NOT EXISTS idx_watch_service ON tool_watches(service_id);
...
CREATE INDEX IF NOT EXISTS idx_alerts_service ON watch_alerts(service_id, created_at DESC);
CREATE INDEX IF NOT EXISTS idx_alerts_time ON watch_alerts(created_at);
